...
if __name__=="__main__":
    ...
    for i in range(1,(num_directories+1)):
        filenames[i]= get_txt_files(f".\{i}")
        lambda_mean = []
        abs_mean = []
        # Cogemos los filenames y abrimos los datos y los guardamos en dataframes
        for file in filenames[i]:
            if file == 'ambos.txt':
                dataframes_dict[f'{i}/{file}'] = pd.read_csv(f".\{i}\{file}")
            else:
                dataframes_dict[file] = pd.read_csv(f".\{i}\{file}", sep='\t', decimal=','
                                                , skiprows=1)
    ...
    eje_x = [0.0001, 0.0002, 0.0003, 0.0004, 0.0005, 0.0006, 0.0007, 0.0008]
    ...
    z = np.polyfit(molaridades, lambdas_max, 1)
    p = np.poly1d(z)
    ...
    # Create layout
    layout1 = go.Layout(
        title='Molaridades medias vs λ máximas(nm)',
        xaxis=dict(title='Molaridades',
                   tickmode='array',
                   tickvals=eje_x,
                   ticktext=eje_x_text),
    ...
    layout2 = go.Layout(
        title='Molaridades medias vs Extinción',
        xaxis=dict(title='Molaridades',
                   tickmode='array',
                   tickvals=eje_x,
                   ticktext=eje_x_text),
    ...
    layout3 = go.Layout(
        title='Molaridades medias vs Área',
        xaxis=dict(title='Molaridades',
                   tickmode='array',
                   tickvals=eje_x,
                   ticktext=eje_x_text),
